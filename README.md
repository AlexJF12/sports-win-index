...
Every league keeps standings; nobody keeps the reading a fan actually feels, which is whether *this* month is a good one to follow your teams, by the only benchmark that means anything — the months that came before it. Banner Month is that reading, taken fresh every morning.
...
1. Create the Bluesky account and note its handle. **`bannermonth.bsky.social`** is the one to try first — it is the site's name and nothing else, which is what a reader has to be able to type from memory after seeing one chart in a timeline. If it's gone, `bannermonthhq`, `thebannermonth` and `bannermonthly` are the fallbacks in that order; avoid anything with a sport or a league in it, since the account covers all four. Once the site has its own domain the handle can move to it (Bluesky verifies handles by DNS), and the `.bsky.social` name stays claimed either way.
